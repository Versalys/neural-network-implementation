...
class Sequential:
    ...
    def forward(self, X, train=True):
        ...
        return self.loss.forward(X, train=train)
    ...
    def backward(self, y):
        ...
        y : np.array
            True labels.
        """
        current_gradient = self.loss.backward(y)
        for module in self.modules[::-1]:
            current_gradient = module.backward(current_gradient)
    ...
    def train(self, dataset):
        ...
        for X, y in dataset: # not using tqdm since it has too much clutter

            # calculate accuracy
            predictions = self.forward(X)

            batch_loss = categorical_cross_entropy(predictions, y)
            loss += batch_loss*len(X)

            accuracy = categorical_accuracy(predictions, y)
            correct += accuracy * len(X)

            self.backward(y)

            num_points += len(X)

        mean_accuracy = correct / num_points
